...
<?= $this->session->flashdata('message');?>
    <div class="sect">
        ...
        <a href="<?= base_url('index.php/Controller/v_input5')?>">Tambah Data <span class="fas fa-plus"></span></a>
    </div>
    <table>
        <tr>
            <th>No.</th>
        ...
        <tr>
            ...
            <td headers="ID"><?= $k->id_p ?></td>
            <td headers="Nama"><?= $k->nama_produk ?></td>
            <td headers="Tanggal"><?= $k->tanggal ?></td>
            <td headers="Jumlah"><?= $k->jumlah_barang ?></td>
            <td headers="Keterangan"><?= $k->keterangan ?></td>
            <td headers="Aksi">
                <a href="<?= base_url().'index.php/Controller/edit_trk/'.$k->id ?>"><i class="fas fa-pen edit"></i></a>
                <a href="<?= base_url().'index.php/Controller/delete_trk/'.$k->id ?>" onclick="return confirm('apakah yakin akan menghapus data ini??')"><i class="fas fa-times delete"></i></a>
            </td>
        </tr>
        <?php }?>
    </table>
...
<div class="row">
<div class="col">
    <form action="<?= base_url('index.php/Controller/filter2')?>" method="POST" target="_blank">
